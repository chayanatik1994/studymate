...
const Home = () => {
  ...
  const fetchPartners = async () => {
    ...
    try {
      ...
      // Show top 6 partners
      const topPartners = (response.data || []).slice(0, 6);
      ...
      <section className="py-16 px-4 md:px-16 bg-gray-50 w-full">
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-600 text-lg">No partners available yet. Be the first to create a profile!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 w-full">
              {partners.map((partner) => (
                <StudyPartnerCard key={partner._id || partner.id} partner={partner} />
              ))}
            </div>
          )}
      </section>
